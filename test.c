# 1 "src/patching/fd_tools.c"
# 1 "<interne>"
# 1 "<command-line>"
# 1 "src/patching/fd_tools.c"
# 28 "src/patching/fd_tools.c"
...
# 689 "/usr/include/netdb.h" 3 4

# 30 "src/patching/fd_tools.c" 2
...
# 32 "src/patching/fd_tools.c" 2

# 1 "src/patching/macros.h" 1
# 31 "src/patching/macros.h"
# 1 "src/common/common_other_tools.h" 1
# 32 "src/common/common_other_tools.h"
void *get_symbol(char *symbol);
# 32 "src/patching/macros.h" 2
# 1 "src/common/common_macros.h" 1
# 33 "src/patching/macros.h" 2
# 34 "src/patching/fd_tools.c" 2
# 1 "src/patching/fd_tools.h" 1
# 33 "src/patching/fd_tools.h"
...
void manage_socket_accesses_on_fdset(enum list_of_fd_access_types access_type, fd_set *fds, fd_set *final_fds);
void remap_changes_to_initial_fdset(enum list_of_fd_access_types access_type, fd_set *initial_fds, fd_set *final_fds);
# 35 "src/patching/fd_tools.c" 2
      ...
      struct timeval *timeout)
{
 ...
 typeof(select) *p_original_func; p_original_func = (typeof(p_original_func)) get_symbol((char *) "select"); if (p_original_func != ((void *)0)) { *&result = p_original_func (nfds, readfds, writefds, errorfds, timeout); } else { *&result = -1; };
...
int get_equivalent_address(struct sockaddr *sa, unsigned int sa_size, int wanted_af, struct sockaddr *new_sa, unsigned int *new_sa_size)
{
 int result;
 ...
 result = getnameinfo(sa, sa_size, host, 128, service, 128, 2);
 if (result == 0)
 {
...
int create_additional_socket(int af, int socktype, struct sockaddr *sa, unsigned int sa_size)
{
 struct sockaddr_storage new_ss;
 struct sockaddr *new_sa = (struct sockaddr *)&new_ss;
 unsigned int new_sa_size = sizeof(new_ss);
 int new_socket_created = -1;
 int on = 1;




 if (get_equivalent_address(sa, sa_size, af, new_sa, &new_sa_size) == 0)
 {
  new_socket_created = socket(af, socktype, 0);
  if (new_socket_created != -1)
  {

   setsockopt(new_socket_created, 1, 2, (char *)&on, sizeof(on));



   if (af == 10)
   {
    setsockopt(new_socket_created, IPPROTO_IPV6, 26, (char *)&on, sizeof(on));
   }
   if (bind(new_socket_created, new_sa, new_sa_size) == -1)
   {
    close(new_socket_created);
    new_socket_created = -1;
   }
  }
  if (new_socket_created != -1)
  {
   if (listen(new_socket_created, 10) == -1)
   {
    close(new_socket_created);
    new_socket_created = -1;
   }
...
int wait_on_two_sockets(int socket1, int socket2)
{
 ...
 do { unsigned int __i; fd_set *__arr = (&socket_list_mask); for (__i = 0; __i < sizeof (fd_set) / sizeof (__fd_mask); ++__i) ((__arr)->__fds_bits)[__i] = 0; } while (0);
 (((&socket_list_mask)->__fds_bits)[((socket1) / (8 * sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((socket1) % (8 * sizeof (__fd_mask)))));
 (((&socket_list_mask)->__fds_bits)[((socket2) / (8 * sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((socket2) % (8 * sizeof (__fd_mask)))));


 nbfds = original_select((socket1>socket2)?(socket1+1):(socket2+1), &socket_list_mask, ((void *)0), ((void *)0), ((void *)0));


 if (((((&socket_list_mask)->__fds_bits)[((socket1) / (8 * sizeof (__fd_mask)))] & ((__fd_mask) 1 << ((socket1) % (8 * sizeof (__fd_mask))))) != 0))
 {
  return socket1;
 }
 else
...
int manage_socket_access_on_fd(enum list_of_fd_access_types access_type, int fd)
{
 ...
 unsigned int sa_size = sizeof(ss);
 int v6only_option;
 ...
 switch(access_type)
 {
  case eAccessType_Accept:
   ...
   getsockopt(fd, 1, 3, (char *)&socktype, &socktype_size);

   getsockname(fd, sa, &sa_size);
   switch(sa->sa_family)
   {
    case 2:

     new_socket_created = create_additional_socket(10, socktype, sa, sa_size);
     break;
    case 10:

     v6only_option_size = sizeof(v6only_option);
     getsockopt(fd, IPPROTO_IPV6, 26, (char *)&v6only_option, &v6only_option_size);
     if (v6only_option == 1)
     {
      new_socket_created = create_additional_socket(2, socktype, sa, sa_size);
     }
     break;
   }
   if (new_socket_created != -1)
   {
    ...
    resulting_fd = wait_on_two_sockets(fd, new_socket_created);
   }
   break;
  default:
...
void manage_socket_accesses_on_fdset(enum list_of_fd_access_types access_type, fd_set *initial_fds, fd_set *final_fds)
{

 if (initial_fds == ((void *)0))
 {
  do { unsigned int __i; fd_set *__arr = (final_fds); for (__i = 0; __i < sizeof (fd_set) / sizeof (__fd_mask); ++__i) ((__arr)->__fds_bits)[__i] = 0; } while (0);
 }
 else
 {
  memcpy(final_fds, initial_fds, sizeof(fd_set));
 }
}
...
void remap_changes_to_initial_fdset(enum list_of_fd_access_types access_type, fd_set *initial_fds, fd_set *final_fds)
{
 ...
 if (initial_fds != ((void *)0))
 {
  memcpy(initial_fds, final_fds, sizeof(fd_set));
 }
}
